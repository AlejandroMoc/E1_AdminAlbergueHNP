...
const UserNewAdmin = (props) => {
  ...
  useEffect(() => {
    fetch('http://192.168.100.81:8008/alldispbeds')
      .then((res) => res.json())
  ...
  useEffect(() => {
    fetch('http://192.168.100.81:8008/allareas')
      .then((res) => res.json())
  ...
  useEffect(() => {
    if (isVisitantePrevio === true) {
      fetch('http://192.168.100.81:8008/allclientinfo', {
        method: 'POST',
  ...
  const handleBtRegistroClick = async () => {
    if (validateFields()) {
      ...
      if (showServices) {
        try {
          await fetch('http://192.168.100.81:8008/registerEntradaUnica', {
            method: 'POST',
        ...
        try {
          await fetch('http://192.168.100.81:8008/registerNewPatient', {
            method: 'POST',
  ...
  useEffect(() => {
    const obtenerEstadoCarnet = async () => {
      try {
        const response = await fetch(`http://192.168.100.81:8008/carnetExist/${carnet}`);
  ...
  { const [huespedCliente, setHuespedCliente] = useState({id_zona:0})
    if (id_cama === undefined){
      ...
      useEffect(() =>{
      fetch('http://192.168.100.81:8008/zonabed/'+props.id_cama)
      .then((res) => res.json())
