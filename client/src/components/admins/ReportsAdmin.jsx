...
const ReportsAdmin = () => {
	...
	const [visitanteSeleccionado, setVisitanteSeleccionado] =
		useState("Visitante");
	...
	const [huespedes, setHuespedes] = useState([
		{ id_huesped: 0, nombre: "", apellidos: "" },
	]); // Estado para almacenar los huéspedes
	const [visitantes, setVisitantes] = useState([
		{ id_visitante: 0, nombre: "", apellidos: "" },
	]); // Estado para almacenar los huéspedes
	...
	useEffect(() => {
		if (startDate === null || endDate === null) {
			...
			fetch(
				`${API_URL}/allusers?startDate=${formattedStartDate}&endDate=${formattedEndDate}`
			)
				.then((res) => res.json())
				.then((allusers) => setData(allusers))
			...
			fetch(
				`${API_URL}/allgeneralhuespedes?startDate=${formattedStartDate}&endDate=${formattedEndDate}`
			)
				.then((res) => res.json())
				.then((allgeneralhuespedes) => setDataHuesped(allgeneralhuespedes))
				.catch((error) => console.error("Error fetching user data:", error));
			...
			fetch(
				`${API_URL}/allgeneralvisitantes?startDate=${formattedStartDate}&endDate=${formattedEndDate}`
			)
				.then((res) => res.json())
				.then((allgeneralvisitantes) => setDataVisitante(allgeneralvisitantes))
				.catch((error) => console.error("Error fetching user data:", error));
			...
			fetch(
				`${API_URL}/allgeneralvetados?startDate=${formattedStartDate}&endDate=${formattedEndDate}`
			)
				.then((res) => res.json())
				.then((allgeneralvetados) => setDataVetado(allgeneralvetados))
				.catch((error) => console.error("Error fetching user data:", error));
			...
			fetch(
				`${API_URL}/allingresos?startDate=${formattedStartDate}&endDate=${formattedEndDate}`
			)
				.then((res) => res.json())
				.then((allingresos) => setDataIngreso(allingresos))
				.catch((error) => console.error("Error fetching user data:", error));
	...
	const generatePDF = useReactToPrint({
		...
		pageStyle:
			"@page {size: auto; margin: 0mm 5mm 0mm 5mm; } @media print {body {-webkit-print-color-adjust: exact; } }",
	...
	const handleServicioChange = (servicio) => {
		//Si se marcan todos los servicios,
		//marcar todas las casillas
		if (servicio === "all") {

			if (allServicesSelected==true){
				setAllServicesSelected(!allServicesSelected);
				setServiciosSeleccionados({
				  desayuno: !allServicesSelected,
				  comida: !allServicesSelected,
				  cena: !allServicesSelected,
				  baño: !allServicesSelected,
				  regadera: !allServicesSelected,
				  deuda: !allServicesSelected,
				  notas_cliente: !allServicesSelected,
				});
			}
			else if (allServicesSelected==false){
				setAllServicesSelected(!allServicesSelected);
				setServiciosSeleccionados({
				  desayuno: !allServicesSelected,
				  comida: !allServicesSelected,
		...
		// 	Object.values(serviciosSeleccionados).every((value) => value)
		//   );
	...
	return (
		<div className="App_minheight App_minpadding user_justified">
				...
				<div className="universal_container_dropdown">
					<Dropdown onSelect={(eventKey) => handleHuespedSelect(eventKey)}>
						<Dropdown.Toggle
							variant="success"
							id="dropdown-basic"
							className="universal_toggle_dropdown"
						>
							{huespedSeleccionado}
						</Dropdown.Toggle>
						<Dropdown.Menu className="reports_dropdown_custommenu">
							{huespedes.map((huesped) => (
								<Dropdown.Item
									...
									eventKey={`${huesped.id_huesped} - ${huesped.nombre}  ${huesped.apellidos}`}
								>
						...
						<Dropdown.Menu className="reports_dropdown_custommenu">
							{visitantes.map((visitante) => (
								<Dropdown.Item
									...
									eventKey={`${visitante.id_visitante} - ${visitante.nombre}  ${visitante.apellidos}`}
								>
				...
				<div className="universal_container_dropdown">
					<Dropdown onSelect={(eventKey) => handleVetadoSelect(eventKey)}>
						...
						<Dropdown.Menu className="reports_dropdown_custommenu">
							{vetados.map((vetado) => (
								<Dropdown.Item
									key={vetado.id_vetado}
									eventKey={`${vetado.id_vetado} - ${vetado.nombre}  ${vetado.apellidos}`}
								>
			...
			<div className="reports_container_services">
				...
				{esServicio && (
					<div className="reports_container_checkbox">
						{/*Checkbox para todos los servicios*/}
						...
						<div className="reports_margin_formcheck">
							<input
								...
								onChange={() => handleServicioChange("all")}
							/>
							<label className="form-check-label universal_label_radio" htmlFor="all">
								<span className="universal_text_HM">Seleccionar todo</span>
							</label>
						</div>
						...
						<Table striped bordered hover>
							...
							<tbody>
								<tr>
									<td>
										<p>
											{dataingreso.fecha_inicio
												? handleDateFormat(dataingreso.fecha_inicio)
												: ""}
										</p>
									</td>
									<td>
										<p>
											{dataingreso.fecha_fin
												? handleDateFormat(dataingreso.fecha_fin)
												: ""}
										</p>
									</td>
									<td>
										<p>${dataingreso.total_pagado}</p>
									</td>
									<td>
										<p>${dataingreso.total_condonado}</p>
									</td>
									<td>
										<p>${dataingreso.ingresos_reales}</p>
									</td>
								</tr>
							</tbody>
						</Table>
