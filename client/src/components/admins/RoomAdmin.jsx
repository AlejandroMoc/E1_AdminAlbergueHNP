...
function update(setInfoM, setInfoH, setInfoA){
  fetch('http://localhost:8008/beds')
  .then((res) => res.json())
...
function Cama({idCama, idCliente, color, iconocama, numCama, nombre, carnet, apellidos, balance, txtBalance, setInfoM, setInfoH, setInfoA, admin}){
  ...
  useEffect(() => {
    ...
    fetch('http://localhost:8008/beds/pagar' , {
      method: 'POST',
  ...
  useEffect(() => {
    if(cliente_UE_RegServicio != 0){
      if(cantidadS1_UE_RegServicio != 0){
        fetch('http://localhost:8008/beds/regServacio' , {
        method: 'POST',
  ...
  useEffect(() => {
    ...
    fetch('http://localhost:8008/beds/regSalida' , {
      method: 'POST',
  ...
  useEffect(() => {
    if (cama_UE_EliminarCama !== 0) {
      fetch('http://localhost:8008/beds/eliminarCama', {
        method: 'POST',
...
const RoomAdmin = () => {
  ...
  */
  ...
  useEffect(() => {
    if(zona != 0){
      fetch('http://localhost:8008/beds/addCama' , {
        method: 'POST',
