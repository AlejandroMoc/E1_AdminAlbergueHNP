...
import MyToastContainer, {
  successToast,
  errorToast,
} from "../universal/MyToast";
        ...
        items={
          adminInfo.admin
            ? [
                {
                  key: "nombre",
                  label: <strong>{name + " " + last_m}</strong>,
                },
                veto
                  ? {
                      key: "noVetar",
                      label: <span style={{ color: "green" }}>Desvetar</span>,
                      icon: (
            ...
            ? [
                {
                  key: "nombre",
                  label: <strong>No Tienes Permisos de Administrador</strong>,
                },
              ]
            : [
                {
  ...
  const constTable = () => {
    return (
      <Table striped bordered hover>
        ...
        <tbody>
          {(filterText.length == 0 ? paginatedData : data).map(
            (item, i) =>
              item.carnet.toUpperCase().includes(filterText) && ( //SE PUEDE MEJORAR, PERO ES FUNCIONAL
                <DP
                  overlay={menu(
                    item.id_cliente,
                    item.nombre_c,
                ...
                >
                  <tr key={i} style={{ background: "#fff" }}>
                    {select_View == 10 ? (
                      <td>
                        <p>{item.nombre_u ? item.nombre_u : "-"}</p>
                      </td>
                    ) : select_View == 9 ? (
                      ""
                    ) : (
                      <td>
                        <p>{item.id_cama ? item.id_cama : "-"}</p>
                      </td>
                    )}
                    <td>
                      {item.nombre_c ? (
                        <Link
                          className="userlist_color_personlink"
                          to={"/infouser/" + item.id_cliente}
                        >
                          {item.nombre_c} {item.apellidos_c}
                        </Link>
                      ) : (
                        "-"
                      )}
                    ...
                    {select_View == 9 ? (
                      <td>
                        <p>
                          {item.l_fecha_u
                            ? handleDateFormat(item.l_fecha_u)
                            : "-"}
                        </p>
                      </td>
                    ) : select_View == 8 ? (
                    ...
                    {select_View != 9 && (
                      <td>
                        <p>{item.lugar_o ? item.lugar_o : "-"}</p>
                      </td>
                    )}
                    {select_View == 9 && (
                      <td>
                        <p>{item.desayuno ? item.desayuno : "-"}</p>
                      </td>
                    )}
                    {select_View == 9 && (
                      <td>
                        <p>{item.comida ? item.comida : "-"}</p>
                      </td>
                    )}
                    {select_View == 9 && (
                      <td>
                        <p>{item.cena ? item.cena : "-"}</p>
                      </td>
                    )}
                    <td>
                      <p>{item.carnet ? item.carnet : "-"}</p>
                    </td>
                    <td>
                      <p>{item.nivel_se ? item.nivel_se : "-"}</p>
                    </td>
                    <td>
      ...
      <div className="userlist_container_flex">
        <div className="userlist_container_upper">
          <div className="universal_container_dropdown universal_container_pickerdate">
            ...
            <Dropdown className="universal_container_dropdown">
              <Dropdown.Toggle
                className="userlist_toggle_dropdown universal_toggle_dropdown"
                variant="success"
                id="dropdown-basic"
              >
                Filtros
          ...
          {/*Apartado para carnet*/}
          <div className="deuda-box-container userlist_padding_formcheck">
            <input
              className="userlist_inputdebt"
              type="text"
              value={filterText}
        ...
        <div className="userlist_container_lower">
          <div>
            {/* Div para viewpicker */}
            <label
              className="userlist_container_radio universal_label_radio"
              key={10}
            >
              <input
