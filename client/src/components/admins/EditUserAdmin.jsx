...
const UserNewAdmin = (props) => {
  ...
  const handleBtRegistroClick = async () => {
    if (validateFields()) {
      ...
      if (showNumbersSelect === false) {
        try {
          await fetch('http://192.168.100.81:8008/updateinfoEntrada', {
            method: 'POST',
        ...
        try {
          await fetch('http://192.168.100.81:8008/updateinfocliente', {
            method: 'POST',
  ...
  useEffect(() => {
    const obtenerEstadoCarnet = async () => {
      try {
        const response = await fetch(`http://192.168.100.81:8008/carnetExistEdit/${carnet}/${id_cliente}`);
